...
License: GPLv2 or later
  ...
  Copyright (C) 2017 1000grad Digital GmbH (info@1000grad.de)
...
require_once("lib/accountmanagerApi.php");
require_once("lib/epaperApikeyApi.php");
require_once("lib/epaperChannelApi.php");

function tgd_epaper_plugin_test_requirements() {
    $class_error = 'notice notice-error';

    if(OPENSSL_VERSION_NUMBER < 0x012080bf) {
        $message = __( 'Please Update SSL. Your Version: ');
        printf( '<div class="%1$s"><p><b>Epaper-Error:</b> %2$s <i>%3$s</i></p></div>',  $class_error ,  $message , OPENSSL_VERSION_TEXT );
    }
...
class TG_Epaper_WP_Plugin {
        ...
        try{
            ...
            foreach ( $oRssItems as $oEntry ){
                ...
                $aNews[] = array(
                    ...
                    'description' => substr($sDescription,0, strpos($sDescription, "</p>")+4),
    ...
    public function adminpage_epaper_channels(){
        ...
        if(!$this->bIsUpdatedAccountmanager){
            $this->showWarning(sprintf('<br><b>%s</b><br><br>%s<br><br>',__("Our WordPress plugin is moving. We will shut down the formerly used server for the Plugin on the 31st of March 2016!"), _("To use the new version it is necessary to perform an update of your plugin by clicking the button below. In the last step you have to go though a new registration.")));
    ...
    public function getEpaperLink($channel = NULL, $epaper = NULL, $aConfig = array()){
        ...
        </a>', $sEpaperLink, $sClass, $img_url, $sParameter);
