    ...
    initUploader:function(){
        jQuery(':file').change(function(){
            if(this.files[0] !== undefined){
                ...
                var size = file.size;
                var type = file.type;
                jQuery(this).parents('form').first().find(':button').trigger('click');
            }
        });
        ...
        jQuery('div#tg_epaper_progress_bar').find('a.tg_cancel_upload').click(function(){
           ...
           TgEpaper.resetProgressBar();
            ...
            jQuery('form.epaper_upload_form').find(':button').click(function(){
                var form = jQuery(this).parent('form')[0];
                var formData = new FormData(form);
                var fileField = jQuery(form).find(':file').first();
                var channel = fileField.attr('channel');
                formData.apikey = jQuery('input#epaperapikey').val();
                jQuery.ajax({
                    url: TGELocalData.restUploadUrl,
                    type: 'POST',
                    xhr: function() { 
                    ...
                    success: function(status, data, response) {
                        ...
                        var pdfId = result.pdfId;
                        var apiKey = jQuery('input#epaperapikey').val();
                        var channel_id = jQuery('input#channel_id_'+channel).val();
                        var oldEpaperId = jQuery('input#epaper_id_'+channel).val();
                        if(parseInt(jQuery('input#tg_upload_cancel_upload').val()) === 0) TgEpaper.startRendering(pdfId, oldEpaperId, channel_id, fileField[0].files[0].name);
                ...
                function progressUpload(e){
                    upload_percent = parseInt((e.loaded/e.total)*100);
